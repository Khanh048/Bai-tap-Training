...
import { CalendarDays, LayoutDashboard, ListChecks, LoaderCircle, LogOut, Menu, Plus, Search, X } from "lucide-react";
...
export function Dashboard() {
  ...
  const previewEnergy = view === "today" && energyPreview?.date === anchorKey ? energyPreview.value : null;
  const themeEnergy = view === "today" ? previewEnergy ?? anchorCheckin?.energy_level ?? null : todayCheckin?.energy_level ?? null;
  const themeBand: EnergyBand | "unset" = themeEnergy === null ? "unset" : energyBand(themeEnergy);
  ...
  const blockingOverlayOpen = onboardingOpen || assistantOpen || formOpen || todoFormOpen || Boolean(completing) || Boolean(deleting);
  const overdueQueue: OverdueItem[] = [...overdueActivities.map((value): OverdueItem => ({ kind: "activity", value })), ...overdueTodos.map((value): OverdueItem => ({ kind: "todo", value }))]; const overdueItem = !loading && !blockingOverlayOpen ? overdueQueue[0] ?? null : null;
  const reminderPresentationSuppressed = blockingOverlayOpen || Boolean(overdueItem);
  ...
  async function saveCheckin(energy: number, note: string) { setSavingCheckin(true); try { const saved = await repository.saveCheckin(anchorKey, energy, note); setCheckins((items) => [...items.filter((item) => item.checkin_date !== saved.checkin_date), saved]); if (anchorKey === todayKey) setTodayCheckin(saved); setEnergyPreview((current) => current?.date === saved.checkin_date ? null : current); } catch (reason) { setError(reason instanceof Error ? reason.message : "Chưa thể lưu check-in."); } finally { setSavingCheckin(false); } }
  ...
  function startTourFromAssistant() {
  ...
  return <div className="dashboard-shell min-h-screen" data-energy-band={themeBand}><aside className={`dashboard-sidebar fixed inset-y-0 left-0 z-40 flex h-dvh w-72 flex-col overflow-y-auto border-r p-5 transition-transform lg:translate-x-0 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}><div className="flex items-center justify-between"><Brand compact /><button className="rounded-xl p-2 lg:hidden" onClick={() => setMenuOpen(false)} aria-label="Đóng menu"><X size={20} /></button></div><nav className="mt-10 space-y-2"><button onClick={() => { setEnergyPreview(null); setView("today"); setAnchorDate(today); setMenuOpen(false); }} className={`sidebar-link ${view === "today" && anchorKey === todayKey ? "sidebar-link-active" : ""}`}><LayoutDashboard size={19} />Hôm nay</button><button onClick={() => { setEnergyPreview(null); setView("month"); setMenuOpen(false); }} className={`sidebar-link ${view === "month" ? "sidebar-link-active" : ""}`}><CalendarDays size={19} />Lịch</button><button data-tour="todo-nav" onClick={() => { setEnergyPreview(null); setView("todos"); setMenuOpen(false); }} className={`sidebar-link ${view === "todos" ? "sidebar-link-active" : ""}`}><ListChecks size={19} />Việc cần làm</button></nav><div className="mt-auto space-y-3 pt-6"><div className="sidebar-profile rounded-2xl p-4"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-sage-700 font-bold text-white">{profile?.display_name.charAt(0).toUpperCase() ?? "B"}</span><div className="min-w-0"><p className="truncate text-sm font-bold text-ink-900">{profile?.display_name ?? "Bạn"}</p><p className={`text-xs font-bold ${batteryTone}`}>{batteryText}</p>{batteryStatus && <p className="text-xs text-ink-500">{batteryStatus}</p>}</div></div></div><button onClick={() => void signOut()} className="sidebar-link"><LogOut size={18} />{configured ? "Đăng xuất" : "Về trang chủ"}</button></div></aside>{menuOpen && <button className="fixed inset-0 z-30 bg-ink-900/30 lg:hidden" onClick={() => setMenuOpen(false)} aria-label="Đóng menu" />}<div className="lg:pl-72"><header className="header-shell sticky top-0 z-20 border-b border-sage-200/80 px-4 py-4 backdrop-blur sm:px-7"><div className="mx-auto flex max-w-7xl items-center gap-3"><button data-tour="mobile-menu" onClick={() => setMenuOpen(true)} className="theme-icon-surface rounded-xl border p-2.5 lg:hidden" aria-label="Mở menu"><Menu size={20} /></button><div className="min-w-0 flex-1"><p className="truncate text-lg font-bold text-ink-900">Chào {profile?.display_name ?? "bạn"}, hôm nay mình vừa sức nhé!</p><p className="hidden text-sm capitalize text-ink-500 sm:block">{formatVietnameseDate(today)}</p></div>{!configured && <DemoBadge />}<div data-tour="header-actions" className="flex shrink-0 items-center gap-2"><button type="button" className="icon-button" onClick={() => setAssistantOpen(true)} aria-expanded={assistantOpen} aria-controls="mascot-assistant-content" aria-label="Mở trợ lý hướng dẫn Koboyo" title="Trợ lý hướng dẫn"><MascotAvatar size={30} staticFrame className="header-help-mascot" /></button><ReminderCenter activities={reminderActivities} todos={todos} presentationSuppressed={reminderPresentationSuppressed} /><button onClick={() => openCreate(today)} className="button-primary hidden sm:inline-flex"><Plus size={18} />Thêm hoạt động</button></div></div></header><main className="mx-auto max-w-7xl px-4 py-6 sm:px-7 sm:py-8">{error && <div role="alert" className="mb-5 flex items-center justify-between rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"><span>{error}</span><button onClick={() => void load()} className="font-bold underline">Thử lại</button></div>}{view === "todos" ? <TodoPanel key={todayKey} todos={todos} today={today} featureState={todoFeature} featureError={todoFeatureError} pendingIds={pendingTodoIds} onRetry={() => void load(false)} onOpenForm={(date, title) => void openTodo(date, undefined, title)} onComplete={completeTodo} onEdit={(todo) => void openTodo(todo.scheduled_date, todo)} onCancel={(todo) => changeTodoStatus(todo, "cancelled")} onRestore={(todo) => changeTodoStatus(todo, "pending")} onDelete={deleteTodo} /> : <><CalendarToolbar view={view} anchor={anchorDate} onViewChange={(nextView) => { setEnergyPreview(null); setView(nextView); }} onAnchorChange={(direction) => { setEnergyPreview(null); setAnchorDate((current) => shiftAnchor(current, view, direction)); }} onAnchorSelect={(date) => { setEnergyPreview(null); setAnchorDate(date); }} onToday={() => { setEnergyPreview(null); setAnchorDate(today); }} /><div className="mb-5 flex flex-col gap-3"><div className="flex flex-col gap-2 xl:flex-row"><label className="relative flex-1"><span className="sr-only">Tìm hoạt động</span><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-400" size={18} /><input value={filters.query} onChange={(event) => setFilters((value) => ({ ...value, query: event.target.value }))} className="field pl-10" placeholder="Tìm theo tên hoạt động…" /></label><div className="grid grid-cols-3 gap-2"><FilterSelect label="Kiểu lịch" value={filters.scheduleType} onChange={(value) => setFilters((current) => ({ ...current, scheduleType: value as ActivityFilters["scheduleType"] }))} options={scheduleTypes.map((item) => [item, scheduleLabels[item]])} /><FilterSelect label="Nhóm" value={filters.category} onChange={(value) => setFilters((current) => ({ ...current, category: value as ActivityFilters["category"] }))} options={activityCategories.map((item) => [item, categoryLabels[item]])} /><FilterSelect label="Trạng thái" value={filters.status} onChange={(value) => setFilters((current) => ({ ...current, status: value as ActivityFilters["status"] }))} options={activityStatuses.map((item) => [item, statusLabels[item]])} /></div></div><div className="flex flex-wrap items-center gap-4">{filtersActive && <button onClick={() => setFilters((value) => ({ ...emptyFilters, query: value.query }))} className="text-xs font-bold text-sage-700 underline">Xoá bộ lọc</button>}</div></div>{view === "today" ? <TodayView anchorKey={anchorKey} activities={anchorActivities} visibleActivityIds={visibleIds} pendingActivityIds={pendingActivityIds} checkin={anchorCheckin} defaultEnergy={defaultEnergy} previewEnergy={previewEnergy} nowIso={nowIso} savingCheckin={savingCheckin} onEnergyPreview={(value) => setEnergyPreview({ date: anchorKey, value })} onSaveCheckin={saveCheckin} onAdd={() => openCreate(anchorDate)} onComplete={setCompleting} onEdit={openEdit} onSkip={skipActivity} onDelete={setDeleting} /> : view === "week" ? <WeekView anchor={anchorDate} activities={activities} visibleActivityIds={visibleIds} pendingActivityIds={pendingActivityIds} checkins={checkins} defaultEnergy={defaultEnergy} now={currentTime} onAdd={openCreate} onComplete={setCompleting} onEdit={openEdit} onSkip={skipActivity} onDelete={setDeleting} /> : <MonthView anchor={anchorDate} activities={activities} visibleActivityIds={visibleIds} checkins={checkins} defaultEnergy={defaultEnergy} now={currentTime} onAdd={(date) => openCreate(date, true)} onEdit={openEdit} />}</>}</main></div>{profile && <OnboardingTour key={profile.id} open={onboardingOpen} profileId={profile.id} onClose={() => setOnboardingOpen(false)} />}<MascotAssistant open={assistantOpen} onClose={() => setAssistantOpen(false)} onStartTour={startTourFromAssistant} /><Modal open={formOpen} onClose={() => setFormOpen(false)} closeDisabled={formBusy} title={editing ? "Chỉnh lại hoạt động" : "Thêm một hoạt động"} description="Sắp lịch theo nhịp năng lượng của bạn" size="lg">{formOpen && <ActivityForm activity={editing} activities={activities} initialEnergy={formInitialEnergy} initialContextAvailable={formInitialContextAvailable} defaultStart={defaultStart} onCancel={() => setFormOpen(false)} onBusyChange={setFormBusy} onLoadDateContext={loadActivityDateContext} onSave={saveActivity} />}</Modal><Modal open={todoFormOpen} onClose={() => setTodoFormOpen(false)} closeDisabled={todoFormBusy} title={editingTodo ? "Chỉnh việc cần làm" : "Thêm việc cần làm"} description="Việc nhỏ, ngày rõ ràng"><TodoForm todo={editingTodo} linkedActivity={todoLinkedActivity} initialTitle={todoInitialTitle} defaultDate={todoDate} onCancel={() => setTodoFormOpen(false)} onBusyChange={setTodoFormBusy} onLoadDateContext={loadActivityDateContext} onSave={saveTodo} /></Modal><EnergyDialog key={completing?.id ?? "energy-closed"} activity={completing} onClose={() => setCompleting(null)} onSave={async (energy) => { if (completing) await updateActivity(completing, { status: "completed", actual_energy_after: energy }); setCompleting(null); }} /><ScopeDialog key={deleting?.id ?? "scope-closed"} activity={deleting} onClose={() => setDeleting(null)} onConfirm={async (scope) => { if (!deleting) return; await deleteActivityAndLinked(deleting, scope); setDeleting(null); }} /><OverdueDialog key={overdueItem ? `${overdueItem.kind}-${overdueItem.value.id}` : "overdue-closed"} item={overdueItem} remaining={overdueQueue.length} onReschedule={handleOverdueReschedule} onCancel={handleOverdueCancel} onKeep={handleOverdueKeep} /></div>;
}
