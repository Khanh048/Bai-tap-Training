...
import { forecastActivities } from "@/lib/energy";
...
export function Dashboard() {
  ...
  const batteryText = todayCheckin ? `Pin hiện tại ${todayCheckin.energy_level}%` : "Chưa check-in hôm nay"; const batteryStatus = todayCheckin ? energyStatus(todayCheckin.energy_level) : "";
  ...
  async function restoreActivity(activity: Activity): Promise<void> {
    await repository.updateActivity(activity, { title: activity.title, category: activity.category, schedule_type: activity.schedule_type, starts_at: activity.starts_at, ends_at: activity.ends_at, expected_impact: activity.expected_impact, actual_energy_after: activity.actual_energy_after, status: activity.status, note: activity.note, overdue_acknowledged_at: activity.overdue_acknowledged_at }, "single");
  }
  async function saveActivity(draft: ActivityDraft, scope: ActionScope) {
    if (editing) {
      const linkedTodo = await linkedTodoFor(editing.id);
      await repository.updateActivity(editing, { title: draft.title, category: draft.category, schedule_type: draft.schedule_type, starts_at: draft.starts_at, ends_at: draft.ends_at, expected_impact: draft.expected_impact, note: draft.note, overdue_acknowledged_at: null }, scope);
      if (linkedTodo) {
        try { await repository.updateTodo(linkedTodo, { title: draft.title, scheduled_date: dateKey(new Date(draft.starts_at)), due_at: draft.starts_at, note: draft.note, overdue_acknowledged_at: null }); }
        catch (reason) { await restoreActivity(editing); throw reason; }
      }
  ...
  async function saveTodo(draft: TodoFormDraft) {
    const activityDraft: ActivityDraft = { title: draft.title, category: "personal", schedule_type: "flexible", starts_at: draft.starts_at, ends_at: draft.ends_at, expected_impact: draft.expected_impact, note: draft.note, recurrence: "none" };
  ...
  function deleteTodo(todo: Todo) { runTodo(todo, async () => { const activity = todo.activity_id ? await repository.getActivity(todo.activity_id) : null; if (activity) await repository.deleteLinkedTodo(todo, activity); else await repository.deleteTodo(todo); await reloadAfterMutation(); }, "Chưa thể xoá việc."); }
  async function deleteActivityAndLinked(activity: Activity, scope: ActionScope) { const linkedTodo = await linkedTodoFor(activity.id); if (linkedTodo) await repository.deleteLinkedTodo(linkedTodo, activity); else await repository.deleteActivity(activity, scope); await reloadAfterMutation(); }
